"""AR level measurements, digital tape runs vs twin, device registration, and live sync."""
import logging
from typing import Optional

from fastapi import APIRouter, Depends, HTTPException

from ar_math import compare_tape_shots, derive_metrics, design_stations_from_spec
from auth import get_current_user
from db import db
from models import (
    ARMeasurement, ARMeasurementCreate, DeviceRegistration, DeviceRegistrationCreate,
    LEVEL_TOLERANCE_IN, TapeRunCreate, TapeRunPreview, TapeShotIn, new_id, now_iso,
)
...
def _shot_metrics(origin: dict, shot: TapeShotIn):
...
async def create_tape_run(payload: TapeRunCreate, user=Depends(get_current_user)):
    try:
        ...
        packed = [_shot_metrics(origin, s) for s in payload.shots]
        ...
        logger.info(
            "tape run saved id=%s beam=%s shots=%s rescan=%s by=%s",
            run_id, payload.beam_id, len(shot_ids), compare.get("rescan_count"), user.get("email"),
        )
