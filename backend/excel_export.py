...
def build_qir(context: dict) -> bytes:
    ...
    for insp in context.get("inspections", []):
        ...
        ws.cell(row=r, column=5, value=insp.get("created_at", "")[:10]).border = BORDER
        r += 1
    _widths(ws)
    return _save(wb)
...
def build_camber(context: dict) -> bytes:
    ...
    _header_row(ws, 5, ["Beam Mark", "Design Camber (in)", "Measured Camber (in)",
                        "Required Str (psi)", "Release Str (psi)", "Date"])
    r = 6
    for c in context.get("camber_readings", []):
        vals = [c.get("beam_mark", ""), c.get("design_camber_in", ""),
                c.get("measured_camber_in", ""), c.get("required_strength_psi", ""),
                c.get("release_strength_psi", ""), c.get("reading_date", "")[:10]]
        for i, v in enumerate(vals):
            ws.cell(row=r, column=1 + i, value=v).border = BORDER
        r += 1
    _widths(ws)
...
def build_crackmap(context: dict) -> bytes:
    ...
    for a in context.get("anomalies", []):
        ...
        vals = [a.get("beam_mark", ""), a.get("type", ""), a.get("severity", ""),
                ...
                a.get("length_in", ""), a.get("note", ""), a.get("created_at", "")[:10]]
        for i, v in enumerate(vals):
            ws.cell(row=r, column=1 + i, value=v).border = BORDER
        r += 1
    _widths(ws)
