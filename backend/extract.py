...
Extract a JSON object with this exact shape (no markdown):
{
  ...
  "web_thick_in": 0,
  "marked_end_id": "",
  "strands": [{"number":1,"row":1,"column":1,"size":"0.5in","detensioning":"straight|draped","draped":false,"soffit_in":2,"offset_in":0,"drape_peak_in":null,"hold_down_y_in":null,"hold_down_stations_ft":[],"debond_me_ft":0,"debond_ue_ft":0}],
  "hold_downs": [{"station_from_marked_end":0,"height":2.5,"type_spec":"Dayton/Richmond H-56-S","quantity_at_station":2,"orientation":"transverse","notes":""}],
    ...
    if data.get("strands"):
        ...
        for s in data["strands"]:
            vision_strands.append(StrandItem(
                ...
                soffit_in=float(s.get("soffit_in") or s.get("hold_down_y_in") or 2),
                offset_in=float(s.get("offset_in") or s.get("x_in") or 0),
                x_in=float(s.get("x_in") or s.get("offset_in") or 0),
                y_in=float(s.get("y_in") or s.get("drape_peak_in") or s.get("soffit_in") or 2),
                drape_peak_in=s.get("drape_peak_in"),
                hold_down_y_in=s.get("hold_down_y_in"),
                hold_down_stations_ft=s.get("hold_down_stations_ft") or [],
