...
# ---------- Finish / Pre-Delivery / Camber ----------
class TestPlantForms:
    @pytest.fixture(scope="class")
    def some_beam_id(self):
        r = requests.post(f"{API}/auth/login", json={"email": ADMIN[0], "password": ADMIN[1]}, timeout=30)
        tok = r.json()["access_token"]
    ...
    def test_camber_create_and_list(self, some_beam_id):
        beam_id, h = some_beam_id
        payload = {
            "beam_id": beam_id,
            "design_camber_in": 1.5,
            "marked_end_in": 0.25,
            "midspan_in": 1.6,
            "unmarked_end_in": 0.3,
            "release_strength_psi": 6200,
            "required_strength_psi": 5500,
            "notes": "TEST_camber",
        }
        r = requests.post(f"{API}/camber-readings", json=payload, headers=h, timeout=30)
        assert r.status_code == 200, r.text
        created = r.json()
        assert created["midspan_in"] == 1.6
        assert created["measured_camber_in"] == 1.6
        r2 = requests.get(f"{API}/camber-readings?beam_id={beam_id}", headers=h, timeout=30)
        assert r2.status_code == 200
        assert any(c["id"] == created["id"] for c in r2.json())

    def test_finish_sheet_create_and_list(self, some_beam_id):
        beam_id, h = some_beam_id
        payload = {
            "beam_id": beam_id,
            "strand_cut_flush": True,
            "strand_recessed": True,
            "strand_grouted": True,
            "hardware_complete": True,
            "surface_finish": "trowel",
            "surface_pass": True,
            "marked_end_id": "ME-TEST-01",
            "marked_end_verified": True,
            "status": "pass",
        }
        r = requests.post(f"{API}/finish-sheets", json=payload, headers=h, timeout=30)
        assert r.status_code == 200, r.text
        created = r.json()
        assert created["marked_end_id"] == "ME-TEST-01"
        r2 = requests.get(f"{API}/finish-sheets?beam_id={beam_id}", headers=h, timeout=30)
        assert r2.status_code == 200
        assert any(s["id"] == created["id"] for s in r2.json())

    def test_pre_delivery_draft(self, some_beam_id):
        beam_id, h = some_beam_id
        payload = {
            "beam_id": beam_id,
            "truck_number": "T-99",
            "destination": "KDOT Yard",
            "qc_signoff": "TEST QC",
            "released": False,
        }
        r = requests.post(f"{API}/pre-delivery", json=payload, headers=h, timeout=30)
        assert r.status_code == 200, r.text
        created = r.json()
        assert created["released"] is False
        r2 = requests.get(f"{API}/pre-delivery?beam_id={beam_id}", headers=h, timeout=30)
        assert r2.status_code == 200
        assert any(p["id"] == created["id"] for p in r2.json())

    @pytest.mark.parametrize("form_type", ["finish", "pre_delivery"])
    def test_export_new_forms(self, auth_headers, form_type):
        r = requests.get(f"{API}/forms/export/{form_type}", headers=auth_headers, timeout=60)
        assert r.status_code == 200, r.text
